...
public class FDEModel {
    ...
    public FDEModel(String path, int fmt) throws Exception {
        ...
        Long search_time = endtime - starttime;
    ...
    private void buildMatrix(boolean mini) {
        if (mini) {
            ...
            for (int i = 0; i < num_OriTabs - 1; i++) {
                XTab t0 = xm.tabs.get(i);
                for (int j = i + 1; j < num_OriTabs; j++) {
                    num = 0;
                    XTab t1 = xm.tabs.get(j);
...
//        生成commonVarsBoolean
        for (int i = 0; i < num_OriTabs - 1; i++) {
            ...
            for (int j = i + 1; j < num_OriTabs; j++) {
                XTab t1 = xm.tabs.get(j);
                ArrayList<Integer> tSM = tabsScopeMatrix[i][j];
    ...
    void build1DScope() {
        ...
        for (int i = 0; i < num_OriTabs - 1; ++i) {
            for (int j = i + 1; j < num_OriTabs; ++j) {
                ArrayList<Integer> aca = tabsScopeMatrix[i][j];
                if (aca != null) {
                    aca.sort(Integer::compareTo);
                    int k = 0;
                    boolean has = false;
                    while (k < addtionTabsVarScopeArray.size() && !has) {
                        ArrayList<Integer> cc = addtionTabsVarScopeArray.get(k);
                        ...
                        if (aca.equals(cc)) {           //顺序可能不同，不能判断是否相等
                            ...
                            tabsIDMatrix[i][j] = k;
                            tabsIDMatrix[j][i] = k;
                            Tuple2 t2 = new Tuple2<Integer,Integer>(i, j);
                    ...
                    if (!has && aca.size() > 1) {
                        Tuple2 t2 = new Tuple2<>(i, j);
                        ArrayList tq = new ArrayList<Tuple2<Integer, Integer>>();
                        tq.add(t2);
                        addtionTabsTabScopeArray.add(tq);
                        addtionTabsVarScopeArray.add(aca);
                        int addLength = addtionTabsVarScopeArray.size() - 1;
